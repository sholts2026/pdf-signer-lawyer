...
function drawStampOnCanvas(canvas, w, color) {
  ...
  // "עו״ד" gets its own large line; remaining lines are smaller
  const titleFontSize = Math.round(w * 0.22);
  const bodyFontSize  = Math.round(w * 0.13);
  const titleLineH    = titleFontSize * 1.5;
  const bodyLineH     = bodyFontSize  * 1.6;
  const padY          = bodyFontSize  * 0.5;
